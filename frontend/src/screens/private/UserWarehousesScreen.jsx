      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {warehouses.map((warehouse) => (
          <Link to={`/user-warehouse-details/${warehouse.id}`} key={warehouse.id} className= "card p-4">
            ...
            <p className="text-gray-100"><strong>Adresse:</strong> {warehouse.address}</p>
            <p className="text-gray-100"><strong>Ville:</strong> {warehouse.town}</p>
            <p className="text-gray-100"><strong>Code Postal:</strong> {warehouse.zip}</p>
            <p className="text-gray-100"><strong>Téléphone:</strong> {warehouse.phone}</p>
          </Link>
        ))}
      </div>
    </div>
