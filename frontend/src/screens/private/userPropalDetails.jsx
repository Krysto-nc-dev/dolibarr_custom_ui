  ...
  return (
    <div className="p-6  min-h-screen">
      ...
      <div className=" p-6 rounded-lg shadow-md">
        ...
        <p className="text-lg text-textColor mb-2"><strong>Statut:</strong> {propal.statut_libelle}</p>
        <p className="text-lg text-textColor mb-2"><strong>Condition de règlement:</strong> {propal.cond_reglement_doc}</p>
        <p className="text-lg text-textColor mb-2"><strong>Raison de la demande:</strong> {propal.demand_reason}</p>
        <p className="text-lg text-textColor mb-2"><strong>Total HT:</strong> {Number(propal.total_ht).toLocaleString('fr-FR', { minimumFractionDigits: 0 })} XPF</p>
        <p className="text-lg text-textColor mb-2"><strong>Total TVA:</strong> {Number(propal.total_tva).toLocaleString('fr-FR', { minimumFractionDigits: 0 })} XPF</p>
        <p className="text-lg text-textColor mb-2"><strong>Total TTC:</strong> {Number(propal.total_ttc).toLocaleString('fr-FR', { minimumFractionDigits: 0 })} XPF</p>
        <p className="text-lg text-textColor mb-2"><strong>Date de création:</strong> {new Date(propal.date_creation * 1000).toLocaleDateString()}</p>
        <p className="text-lg text-textColor mb-2"><strong>Date de validation:</strong> {new Date(propal.date_validation * 1000).toLocaleDateString()}</p>
